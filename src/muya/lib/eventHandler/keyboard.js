...
class Keyboard {
  ...
  inputBinding () {
    ...
    const inputHandler = event => {
      ...
      const paragrapha = findNearestParagraph(node)
      const mtTagNode = node && node.classList.contains('ag-mttag') ? node : false
      if (
        paragrapha &&
        mtTagNode
      ) {
        const reference = getParagraphReference(mtTagNode, paragrapha.id)
        eventCenter.dispatch('muya-mttag-picker', {
          reference,
          node: mtTagNode,
          cb: item => {
            contentState.setMttag(mtTagNode, item.displayName)
          }
        })
      }
      if (!mtTagNode) {
